...
class _ParentFeesScreenState extends State<ParentFeesScreen> {
  ...
  Future<void> _handleVoucherUpload(
    ...
    setState(
      () => _loadingVoucherId = installmentIndex != null
          ? "${voucher['id']}_$installmentIndex"
          : voucher['id'],
    );
    ...
    try {
      ...
      final allVouchersSnapshot = await FirebaseFirestore.instance
          ...
          .collection('vouchers')
          .get();
      final allVouchers = allVouchersSnapshot.docs
          .map((d) => d.data())
          .toList();

      double expectedAmount = 0;
      double base =
          double.tryParse(voucher['baseFee']?.toString() ?? '0') ?? 0.0;
      double add =
          double.tryParse(voucher['additionalCharge']?.toString() ?? '0') ??
          0.0;
      double pen = _isOverdue(voucher['dueDateRaw'])
          ? (double.tryParse(voucher['latePenalty']?.toString() ?? '0') ?? 0.0)
          : 0.0;

      if (installmentIndex != null && voucher['installments'] != null) {
        final inst = (voucher['installments'] as List)[installmentIndex];
        expectedAmount =
            double.tryParse(inst['amount']?.toString() ?? '0') ?? 0.0;

        bool overdue = _isOverdue(inst['dueDateRaw'] ?? voucher['dueDateRaw']);
        if (overdue &&
            installmentIndex == _getFirstUnpaidIndex(voucher['installments'])) {
          double penalty =
              double.tryParse(voucher['latePenalty']?.toString() ?? '0') ?? 0.0;
      ...
      final result = await OcrService.validateVoucher(
        ...
        currentInstallmentNumber: installmentIndex != null
            ? installmentIndex + 1
            : null,
      ...
      if (result.success) {
        ...
        String? secureUrl = await CloudinaryService.uploadDartIoFile(
          imageFile,
        ...
        if (installmentIndex != null) {
          ...
          if (result.transactionId != null) {
            installments[installmentIndex]['transactionId'] =
                result.transactionId;
          }

          bool allPaid = installments.every(
        ...
        if (mounted) {
          ScaffoldMessenger.of(context).showSnackBar(
            const SnackBar(
              content: Text("Success! Voucher verified and marked as Paid."),
      ...
      builder: (dialogContext) => AlertDialog(
        ...
        actions: [
          ...
          ElevatedButton(
            onPressed: () async {
              Navigator.pop(dialogContext);
              await _submitForManualVerification(
                imageFile,
                voucher,
                adminId,
                installmentIndex: installmentIndex,
  ...
  Future<void> _submitForManualVerification(
    File imageFile,
    Map<String, dynamic> voucher,
    String adminId, {
    int? installmentIndex,
    ...
    try {
      ...
      if (secureUrl != null) {
        ...
        PushNotificationDispatcher.sendNotificationToTopic(
          ...
          body:
              'Voucher uploaded by $studentName ($rollNo) for $month requires manual verification.',
          additionalData: {
        ...
        if (mounted) {
          ScaffoldMessenger.of(context).showSnackBar(
            const SnackBar(
              content: Text("Submitted for manual verification."),
              ...
              body: StreamBuilder<QuerySnapshot>(
                ...
                builder: (context, snapshot) {
                  ...
                  var unpaidVouchers = vouchers
                      ...
                      .toList();
                  var paidVouchers = vouchers
                      .where((v) => v['status'] == 'paid')
                      .toList();

                  String currentMonthKey = DateFormat('MM-yyyy').format(now);
                  var currentMonthVouchers = vouchers
                      .where((v) => v['monthYear'] == currentMonthKey)
                      .toList();
                  ...
                  double totalUnpaid = currentMonthVouchers.fold(0.0, (sum, v) {
                    ...
                    double base =
                        double.tryParse(v['baseFee']?.toString() ?? '0') ?? 0.0;
                    double add =
                        double.tryParse(
                          v['additionalCharge']?.toString() ?? '0',
                        ) ??
                        0.0;
                    double pen = _isOverdue(v['dueDateRaw'])
                        ? (double.tryParse(
                                v['latePenalty']?.toString() ?? '0',
                              ) ??
                              0.0)
                        : 0.0;
                  ...
                  double totalPaid = currentMonthVouchers.fold(0.0, (sum, v) {
                    ...
                    if (v['status'] == 'paid') {
                      double base =
                          double.tryParse(v['baseFee']?.toString() ?? '0') ??
                          0.0;
                      double add =
                          double.tryParse(
                            v['additionalCharge']?.toString() ?? '0',
                      ...
                      double pen =
                          double.tryParse(
                            v['latePenaltyApplied']?.toString() ?? '0',
                          ) ??
                          0.0;
                      return sum + (base + add + pen);
                    }
                                        ...
                                        Padding(
                                          ...
                                          child: Column(
                                            ...
                                            children: [
                                              if (overdue &&
                                                  penalty > 0 &&
                                                  installmentCards.isEmpty) ...[
                                                _buildLateFeeWarning(
                                                  penalty,
                                                  isUrdu,
                                                ),
                                                const SizedBox(height: 8),
                                              ],
                                              _buildUnpaidInstallmentCard(
                                                v,
                                                inst,
                                                i,
                                                isUrdu,
                                        ...
                                        for (
                                          ...
                                          widgets.add(
                                            Padding(
                                              ...
                                              child: _buildPaidInstallmentCard(
                                                v,
                                                installments[i]
  ...
  Widget _buildUnpaidVoucherCard(Map<String, dynamic> voucher, bool isUrdu) {
    ...
    double additional =
        double.tryParse(voucher['additionalCharge']?.toString() ?? '0') ?? 0.0;
    double penalty =
        double.tryParse(voucher['latePenalty']?.toString() ?? '0') ?? 0.0;
    ...
    String installmentLabel = installments.isNotEmpty
        ? (installments[0]['label'] ?? 'Installment')
        : 'Standard Voucher';
            ...
            children: [
              ...
              _buildBadge(
                (voucher['status'] == 'paid' || voucher['status'] == 'Paid')
                    ...
                    : (voucher['status'] == 'pending_manual'
                          ? Translations.get('Review', isUrdu)
                          : Translations.get('Pending', isUrdu)),
                (voucher['status'] == 'paid' || voucher['status'] == 'Paid')
                    ? Colors.green
                    : (voucher['status'] == 'pending_manual'
                          ? Colors.orange
            ...
            Column(
              children: [
                Container(
                  ...
                  child: Center(
                    child: Text(
                      Translations.get('Pending Admin Review', isUrdu),
                      style: const TextStyle(
                        color: Colors.orange,
                        fontWeight: FontWeight.bold,
                      ),
                ...
                SizedBox(
                  ...
                  child: OutlinedButton.icon(
                    onPressed: _loadingVoucherId != null
                        ? () {}
                        : () => _handleVoucherUpload(voucher),
  ...
  Widget _buildUnpaidInstallmentCard(
    Map<String, dynamic> voucher,
    ...
    if (index == 1 && displayDate == dueDate) {
      try {
        ...
        if (parts.length == 3) {
          ...
          displayDate =
              "${extDate.day.toString().padLeft(2, '0')}/${extDate.month.toString().padLeft(2, '0')}/${extDate.year}";
        }
      } catch (e) {}
    }
  ...
  Widget _buildPaidVoucherCard(Map<String, dynamic> voucher, bool isUrdu) {
    ...
    double additional =
        double.tryParse(voucher['additionalCharge']?.toString() ?? '0') ?? 0.0;
    double penalty =
        double.tryParse(voucher['latePenaltyApplied']?.toString() ?? '0') ??
        0.0;
  ...
  Widget _buildPaidInstallmentCard(
    Map<String, dynamic> voucher,
    Map<String, dynamic> inst,
    int index,
    bool isUrdu,
    ...
    dynamic paidDateRaw =
        inst['paidAt'] ?? inst['paymentDate'] ?? voucher['updatedAt'];
    String paidDate = 'N/A';
    if (paidDateRaw is Timestamp) {
      paidDate = DateFormat('dd/MM/yyyy').format(paidDateRaw.toDate());
    }
